...
Original file is located at
    https://colab.research.google.com/drive/1NKxfnqQHEGdQZBm2wspH1lxRnsYkqN5G
"""
...
MINI_BATCH_SIZE = 10
DISCOUNT_FACTOR = 0.9
LEARNING_RATE_OF_NN = 0.01
UPDATE_TARGET_WEIGHTS_AFTER_EVERY_STEPS = 5
TOTAL_EPISODES = 100
...
#one hot encoding of station number
def convert_station_to_vector(station):
    vector_rep = np.zeros(rewards.shape[0])
...
for i in range(q_values.shape[0]):
    state = convert_station_to_vector(i)
    x = agent.model.predict(np.array(state).reshape(-1,*state.shape))
...
while len(path)!=13:
  ...
  action_index_in_remaining_states = np.argmax(np.delete(q_values[path[-1]], path))
